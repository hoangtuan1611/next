...
export default function Schedule() {
  ...
  const teacherCode = typeof window !== 'undefined' ? localStorage.getItem('TeacherCode') ?? '000.000.00000' : '000.000.00000'
      ...
      <div className="min-h-screen bg-white">
        ...
        <div className="flex items-center justify-between border-b border-gray-100 px-4 py-2">
          ...
          <h1 className="text-sm">
            Thời khoá biểu giảng viên: <span className="font-medium">{teacherName}</span>
          </h1>
        </div>
                          ...
                          {schedule.length > 0
                            ? schedule.map((item, idx) => (
                                <div key={idx} className="space-y-1">
                                  ...
                                  <p className="text-xs text-gray-500">- Mã HP: {item.schedule.classId}</p>
                                  <p className="text-xs text-[#1677FF]">- Lớp: {item.schedule.className}</p>
                                  <p className="text-xs text-gray-500">
                                    - Tiết: {item.periodBegin} → {item.periodEnd}
                                  </p>
                                  <p className="text-xs text-gray-500">- Đã dạy: {item.schedule.maxStudents} Tiết</p>
                                  <p className="text-xs text-gray-500">- Phòng: {item.room}</p>
                                </div>
                              ))
                            : null}
                        </td>
